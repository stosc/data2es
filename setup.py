...
setup(
    name = pack.__serverName__,      #这里是pip项目发布的名称
    ...
    platforms = "any",
    install_requires = ["pymysql","croniter","argparse","elasticsearch","pyhocon"],          #这个项目需要的第三方库
    classifiers=[        
        "Programming Language :: Python :: 3",
    ],
    entry_points={
        'console_scripts': [
            '%s = pack.main:run'%(pack.__serverName__),#把业务模块打包为可系统执行的命令
            '%s = pack.maind:run'%(pack.__daemonName__),
        ]
    },
    python_requires='>=3.6',
)
